...
export default class MailsyncBridge {
  ...
  async forceRelaunchClient(account: Account) {
    const keys = await KeyManager._getKeyHash();
    console.log("mailsync-bridge forceRelaunchClient Account:" + JSON.stringify(account) + " keys:"+ JSON.stringify(keys) );
  ...
  async _launchClient(account: Account, keys, { force }: { force?: boolean } = {}) {
    ...
    console.log("mailsync-bridge _launchClient fullAccountJSON:"+ JSON.stringify(fullAccountJSON) + " identity:" + JSON.stringify(IdentityStore.identity()));
    client.sync();
  ...
  _onIncomingMessages = msgs => {
    for (const msg of msgs) {
      console.log("mailsync-bridge _onIncomingMessages msg:" + JSON.stringify(msg));
      if (msg.length === 0) {
        continue;
      }
      if (msg[0] !== '{') {
