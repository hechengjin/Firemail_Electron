...
export class MailsyncProcess extends EventEmitter {
  ...
  constructor({ configDirPath, resourcePath, verbose }) {
    ...
    console.log("mailsync constructor:" + this.binaryPath);
  }
  ...
  _showStatusWindow(mode) {
    ...
    console.log("mailsync _showStatusWindow:" + `file://${this.resourcePath}/static/db-${mode}.html`);
  ...
  _spawnProcess(mode) {
    ...
    the channel. */
    if (this._proc.stdin) {
      console.log("_spawnProcess setDefaultEncoding utf-8");
    ...
    return new Promise<{ response: any; buffer: Buffer }>((resolve, reject) => {
      ...
      if (this._proc.stderr) {
        this._proc.stderr.on('data', data => {
          console.log("_spawnAndWait on stderr data:" + data)
          buffer += data;
          if (onData) onData(data);
        });
      }
      ...
      this._proc.on('error', err => {
        console.log("_spawnAndWait on error err:" + err)
        reject(err);
      });

      this._proc.on('close', code => {
        console.log("_spawnAndWait on close code:" + code)
        ...
        try {
          ...
          if (code === 0) {
            console.log("_spawnAndWait on close resolve:" + JSON.stringify(response))
  ...
  async migrate() {
    try {
      ...
      console.log("migrate catch err: " + err);
      this._closeStatusWindow();
      throw err;
    }
  }
