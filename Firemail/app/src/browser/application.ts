...
export default class Application extends EventEmitter {
  ...
  openWindowsForTokenState() {
    ...
    console.log("application accounts:" + JSON.stringify(accounts) + " hasIdentity:" + hasIdentity);

    if (hasAccount && hasIdentity) {
      this.windowManager.ensureWindow(WindowManager.MAIN_WINDOW);
    } else {
      console.log("openWindowsForTokenState onboarding");
  ...
  handleEvents() {
    ...
    this.on('application:show-main-window', () => {
      console.log("application.ts show-main-window");
      this.openWindowsForTokenState();
    });
