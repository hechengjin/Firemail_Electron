...
const setupConfigDir = args => {
  let dirname = 'Firemail';
  if (args.devMode) {
    dirname = 'Firemail-dev';
  }
  if (args.specMode) {
    dirname = 'Firemail-spec';
  }
...
const declareOptions = argv => {
  ...
  options.usage(
    `Firemail\n\nUsage: Firemail [options]\n\nRun Firemail: The open source extensible email client\n\n\`firemail --dev\` to start the client in dev mode.\n\n\`firemail --test\` to run unit tests.`
  );
  options
    .alias('d', 'dev')
    .boolean('d')
  ...
  options
    ...
    .string('c')
    .describe('c', 'Override the path to the Firemail configuration directory');
  options
  ...
  options
    .alias('b', 'background')
    .boolean('b')
    .describe('b', 'Start Firemail in the background');
  return options;
};
...
const parseCommandLine = argv => {
  ...
  for (const arg of args._) {
    ...
    if (arg.startsWith('mailto:') || arg.startsWith('firemail:')) {
      urlsToOpen.push(arg);
...
const start = () => {
  app.setAppUserModelId('com.squirrel.firemail.firemail');
